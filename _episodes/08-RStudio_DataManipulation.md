...
# Instead, please edit 08-RStudio_DataManipulation.md in _episodes_rmd/
...
```
## Error in pregnant_lactate_sample %>% filter(adj.P.Val <= 0.05): could not find function "%>%"
```
...
```
## Error in virgin_pregnant_sample %>% filter(adj.P.Val <= 0.05): could not find function "%>%"
```
...
```
## Error in pregnant_lactate_sample_filt2 %>% filter(ENTREZID %in% virgin_pregnant_sample_filt2$ENTREZID): could not find function "%>%"
```
...
```
## Error in eval(expr, envir, enclos): object 'preg_lac_overlap2' not found
```
...
```
## Error in pregnant_lactate_sample %>% filter(adj.P.Val <= 0.05) %>% filter(ENTREZID %in% : could not find function "%>%"
```
...
```
## Error in eval(expr, envir, enclos): object 'preg_lac_overlap3' not found
```
